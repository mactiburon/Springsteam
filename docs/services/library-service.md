...
Servicio de **biblioteca personal**. Cada usuario tiene su lista de juegos con metadatos (`isFavorite`, `hoursPlayed`). Mantiene una **proyección local del catálogo de juegos** sincronizada vía Kafka para poder resolver las respuestas con el juego anidado sin llamadas síncronas a game-service.
...
3. **Proyección local** de juegos: consume eventos `game.created`/`game.updated`/`game.deleted` y sincroniza la tabla local `games` (`GameProjectionService`)
4. **Publicación** de eventos `library.added`/`library.updated`/`library.removed` en el topic `library-events`
...
│   ├── LibraryResponse.kt         ← respuesta con game anidado (GameResponse)
...
├── LibraryEntry.kt                ← entidad JPA (ManyToOne → Game, FK CASCADE)
