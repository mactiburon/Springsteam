...
class LibraryControllerIntegrationTest {
    ...
    private fun registerAndLogin(prefix: String = "player"): Session {
    ...
    private fun createGame(name: String, token: String): Long {
    ...
    fun `anadir juego a la biblioteca devuelve 201 con el juego anidado`() {
        val session = registerAndLogin("libA")
        val gameId = createGame("Almas_$suffix", session.token)
    ...
    fun `anadir duplicado devuelve 409`() {
        val session = registerAndLogin("libB")
        val gameId = createGame("UnicoLib_$suffix", session.token)
    ...
    fun `anadir con juego inexistente devuelve 404`() {
        val session = registerAndLogin("libC")
        mockMvc.perform(
            post("/api/library")
                .header(HttpHeaders.AUTHORIZATION, "Bearer ${session.token}")
                .contentType(MediaType.APPLICATION_JSON)
    ...
    fun `anadir sin gameId devuelve 400`() {
        val session = registerAndLogin("libD")
        mockMvc.perform(
            post("/api/library")
                .header(HttpHeaders.AUTHORIZATION, "Bearer ${session.token}")
                .contentType(MediaType.APPLICATION_JSON)
    ...
    fun `obtener entrada devuelve 200`() {
        val session = registerAndLogin("libE")
        val gameId = createGame("Detalle_$suffix", session.token)
    ...
    fun `entrada inexistente devuelve 404`() {
        val session = registerAndLogin("libF")
    ...
    fun `listar biblioteca vacia devuelve lista vacia`() {
        val session = registerAndLogin("libG")
        createGame("CatLibUno_$suffix", session.token)
        createGame("CatLibDos_$suffix", session.token)
    ...
    fun `listar biblioteca con juegos devuelve los anadidos`() {
        val session = registerAndLogin("libH")
        val gameId = createGame("Tengo_$suffix", session.token)
        addGame(session.token, gameId)
        mockMvc.perform(get("/api/library").header(HttpHeaders.AUTHORIZATION, "Bearer ${session.token}"))
            .andExpect(status().isOk)
    ...
    fun `filtrar por favoritos devuelve solo los marcados`() {
        val session = registerAndLogin("libI")
        val favGame = "Fav_$suffix"
        val normalGame = "Normal_$suffix"
        addGame(session.token, createGame(favGame, session.token), isFavorite = true)
        addGame(session.token, createGame(normalGame, session.token))
    ...
    fun `buscar por nombre de juego dentro de la biblioteca`() {
        val session = registerAndLogin("libJ")
        val match = "Quiero_$suffix"
        val other = "Otra_$suffix"
        addGame(session.token, createGame(match, session.token))
        addGame(session.token, createGame(other, session.token))
    ...
    fun `marcar favorito y horas se actualizan por separado`() {
        val session = registerAndLogin("libK")
        val gameId = createGame("Editable_$suffix", session.token)
        addGame(session.token, gameId)
        mockMvc.perform(
    ...
    fun `actualizar entrada inexistente devuelve 404`() {
        val session = registerAndLogin("libL")
        mockMvc.perform(
    ...
    fun `horas negativas devuelven 400`() {
        val session = registerAndLogin("libM")
        val gameId = createGame("Negativo_$suffix", session.token)
        addGame(session.token, gameId)
        mockMvc.perform(
            put("/api/library/$gameId")
                .header(HttpHeaders.AUTHORIZATION, "Bearer ${session.token}")
    ...
    fun `borrar entrada devuelve 204 y deja de existir`() {
        val session = registerAndLogin("libN")
        val gameId = createGame("BorraLib_$suffix", session.token)
        addGame(session.token, gameId)
    ...
    fun `borrar entrada inexistente devuelve 404`() {
        val session = registerAndLogin("libO")
